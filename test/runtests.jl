...
let rex = r"^(\e\[\?25l)([◒◐◓◑◒◐◓◑][\b]){4,16}([\b ])*(\e\[0J\e\[\?25h)$"
	regex_test(rex, :( @spinner sleep(1) ) )
end

let rex = r"^(\e\[\?25l)([abcdefg][\b]){4,16}([\b ])*(\e\[0J\e\[\?25h)$"
...
let rex = r"^(\e\[\?25l)([⊙◡][\b])*([\b ]){4,16}(\e\[0J\e\[\?25h)$"
	regex_test(rex, :( @spinner :blink sleep(1) ) )
end

let rex = r"^(\e\[\?25l)([🕐🕑🕒🕓🕔🕕🕖🕗🕘🕙🕚🕛][\b][\b]){4,16}([\b ])*(\e\[0J\e\[\?25h)$"
